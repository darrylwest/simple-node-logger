...
var SimpleLogger = function(options) {
    ...
    /**
     * create a rolling file appender and add it to the appenders list
     * @param opts
     * @returns {the}
     */
    ...
    this.startRefreshThread = function() {
        if (fs.existsSync( loggerConfigFile ) && dash.isNumber( refresh )) {
            var t = Math.max( 10 * 1000, refresh );
            refreshId = createInterval( manager.readConfig, t);
            // console.log('start the read config thread: ', refreshId, ' t: ', t);
        }
    };
    ...
    this.readConfig = function() {
        var callback = function(err, buf) {
            ...
            if (conf.loggers) {
                // find each logger and set the level
            }
        };
